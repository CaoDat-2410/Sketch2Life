...
- Independent review status: **NOT YET PERFORMED** (this document authors the successor content only; see the "Required independent review" note under section 12)
- Supersedes: `plan/P2_T4_CONTRACT_FREEZE_DRAFT.md` revision 11, approved and committed at
  `18d0c33d35431ca96a76692a68c6b992098699e7`, normalized SHA-256
  `be96b32aa675b7b6e46eea30effb2dbb91c718dc68ba7ce36d4d627ad6058ee2`. Revision 11 remains
  byte-identical at its own path as immutable G1 history; this document does not edit it and
  is a new, separate, standalone file.
...
Revision 11 was approved as G1 at commit `18d0c33d35431ca96a76692a68c6b992098699e7` and its
companion package (revision 15) was approved as G2 for the exact seven-file offline
implementation, which was then implemented and committed at
`064ba62f32f1ffb964bc2208577eb0650b98e26a` (parent `d9a13d2c51a16702c705795a3c7b497b61d945c0`,
the G2 approval-record commit). A post-checkpoint contract audit
(`plan/P2_T4_MATCH_VIEW_REMEDIATION_PLAN.md`) verified a **VERIFIED_DEFECT**: revision 11's
Vision-envelope row for `policy_match_view_version` states "successful fusion requires the
declared v2 match view" (revision 11, section 5.2), but revision 11 gives that requirement no
pipeline placement, no closed rejection encoding, and no fixture obligation, and its own T4
policy literal (`vision_policy_match_view-v2`, underscored) differs from the upstream
declaration token (`vision-policy-match-view-v2`, hyphenated, `VISION_POLICY_MATCH_VIEW_VERSION`
in `backend/src/sketch2life/contracts/schemas/vision.py:16`). Because revision 11 states the
requirement without defining conforming enforcement, checkpoint `064ba62` performs no check, and
a schema-valid `VisionUnderstandingSuccessV1` carrying any other non-empty match-view token
reaches `FUSED`. This gap cannot be closed by interpretation of revision 11 alone; it requires
this successor.

The project owner recorded five decisions for this successor, authoritatively in
`approvals/TASK_APPROVAL.md` (2026-09-15 P2-T4 match-view successor decisions and package-issuance
authorization entry):
...
- **MV-2 = S2.** Enforce the invariant only for `VisionUnderstandingSuccessV1`. A
  `VisionUnderstandingFailureV1` is not subject to this check.
- **MV-3 = T1.** Compare the observed `policy_match_view_version` for exact byte equality with
  the upstream constant `VISION_POLICY_MATCH_VIEW_VERSION` (`vision-policy-match-view-v2`),
  imported from `sketch2life.contracts.schemas.vision`. The T4 policy literal
  (`vision_policy_match_view-v2`) and `fusion_policy_config_hash` are unchanged; the policy
  literal continues to name the recipe, and the admissibility comparison is against the upstream
  declaration token, not the policy literal.
- **MV-4 = V2.** Keep the fused-result identity `P2T4.P2T4FusedResultV1@1.0` unchanged. Replace
  the outer rejection contract with a new major identity, `P2T4.P2T4FusionInputRejectionV2@2.0`,
  which supersedes `P2T4.P2T4FusionInputRejectionV1@1.0` for every outer-boundary rejection. No
  code path may emit a `P2T4FusionInputRejectionV1` instance once this successor is the approved
  contract, and the outer boundary's return type is `P2T4FusionInputRejectionV2` only — never a
  runtime union of the two rejection versions.
- **MV-5.** Issue standalone successor artifacts at new paths (this file, and the companion
  package revision 16) rather than editing revision 11/15 in place. Revision 11 and revision 15
  remain byte-identical, immutable G1 history at their existing paths.

Recording these five decisions, and issuing this successor for independent review, does not
itself freeze this successor, approve implementation, or authorize evidence. A separate owner
successor-freeze approval (a new "G1 successor" gate) and a separate implementation-remediation
approval remain required before any of the seven paths listed in section 10.2 may be edited.
...
G3-G5 offline-core checkpoint (parent `d9a13d2c51a16702c705795a3c7b497b61d945c0`). That tree
still contains the current FEAT-018 P2-T2 implementation, including the FEAT-018-owned,
...
at `11468d3a5a327697a491f09251a3210987337da0`. None of the FEAT-018/FEAT-020 facts below changed
between revision 11's review base (`d706d88a70c6a9136e397bea10d29f96bafd190b`) and this
successor's review base; only the P2-T4 offline core itself (the seven files) is new in the tree.
...
| Failure shape | Top-level `UPSTREAM_FAILURE` with typed ASR/Vision/BOTH failure references, or a separate input rejection (now `P2T4FusionInputRejectionV2`) | Typed `RawFailureV1` with `RawFailureCode` in the `FAILED` branch; provenance is optional on failure and status semantics differ |
...
This successor's status is exactly `HOLD - NOT APPROVED` pending independent successor-freeze
review and then a separate owner freeze decision. This is not a contract freeze or
implementation approval. The future remediation-implementation source commit is
...
B3c `NOT_MEASURED` null confidence. Revision 11 additionally recorded the owner-selected output
identity, per-segment matching boundary, canonical coordinate and reference rules, decimal-string
policy representation, ambiguous-region handling, and mixed-positive/refuting handling, none of
which this successor reopens (see section 8, "Fused-result and unrelated fusion semantics are
unchanged"). This successor additionally records the MV-1 through MV-5 decisions in section 1.0
above and the match-view admissibility invariant in section 3.
...
| Fusion output | `P2T4.P2T4FusedResultV1@1.0` | `contract_name="P2T4FusedResultV1"`, `contract_version="1.0"` | `P2T4FusedResultV1`, with only `FUSED` or `UPSTREAM_FAILURE` (**unchanged from revision 11**) |
| Safe input rejection | `P2T4.P2T4FusionInputRejectionV2@2.0` | `contract_name="P2T4FusionInputRejectionV2"`, `contract_version="2.0"` | `P2T4FusionInputRejectionV2`, outside the fused-result union (**MV-4: supersedes `P2T4.P2T4FusionInputRejectionV1@1.0`**) |
...
function is entered. Under this successor it may raise the typed error carrying a
`P2T4FusionInputRejectionV2` on an admissibility or correlation failure, exactly as revision 11
specified for `P2T4FusionInputRejectionV1`.
...
3. **P2-T4 admissibility invariants:** after strict upstream validation, require every
   T4-only invariant, in this exact order:
   1. **ASR duplicate-index invariant (unchanged from revision 11):** `For AsrSuccessV1, all
      AsrSegmentV1.index values MUST be unique.`
   2. **Vision match-view invariant (new in this successor, MV-1/MV-2/MV-3):** `For
      VisionUnderstandingSuccessV1, policy_match_view_version MUST equal
      "vision-policy-match-view-v2" exactly` — that is, exact byte equality with the upstream
      constant `VISION_POLICY_MATCH_VIEW_VERSION` declared in
      `backend/src/sketch2life/contracts/schemas/vision.py:16`. This invariant does not widen,
      redefine, or add a new validator to the upstream Vision V1 model; the comparison is a T4
      admissibility check against an already-validated field. It applies only to
      `VisionUnderstandingSuccessV1` (MV-2 = S2): a `VisionUnderstandingFailureV1` is not subject
      to it and proceeds only through the earlier identity/strict-validation gates that already
      apply to it.

   Vision observation-ID uniqueness is already enforced by the upstream V1 schema and is not
   redundantly reimplemented by T4.
4. **Correlation equality:** require `asr.correlation_id == vision.correlation_id`. Both
   admissibility invariants above precede this stage.
5. **Typed upstream status:** both successes use the fusion path; one or both typed failures
   use the typed upstream-failure path. Both admissibility invariants above precede this stage.
...
Vision — including within admissibility, where the ASR duplicate-index invariant is checked
before the Vision match-view invariant. Strict upstream-contract validation is distinct from the
T4 admissibility stage: the current upstream `AsrSuccessV1` schema permits duplicate segment
indexes, and the current upstream Vision V1 schema permits any non-empty
`policy_match_view_version`, so a strictly valid duplicate-index ASR result and a strictly valid
non-canonical-match-view Vision success are each rejected only at the T4 admissibility stage, not
at strict validation.

## 4. Safe typed input rejection

`P2T4FusionInputRejectionV2` is a separate terminal result and is never represented as
an additional `P2T4FusedResultV1.status`. It supersedes `P2T4FusionInputRejectionV1`
(MV-4): every outer-boundary rejection under this successor — for every phase, not only the new
match-view case — uses `contract_name=P2T4FusionInputRejectionV2` and `contract_version=2.0`.
No code path may emit a `P2T4FusionInputRejectionV1` instance, and the outer boundary's return
type is `P2T4FusionInputRejectionV2` only; it must never be a runtime union of the two rejection
versions. It is strict, frozen, has no arbitrary map, and contains exactly these fields:

| Field | Type and requiredness | Allowlist/invariant |
|---|---|---|
| `contract_name` | required string | literal `P2T4FusionInputRejectionV2` |
| `contract_version` | required string | literal string `2.0` |
...
| `field_code` | required string | `NONE`, `CONTRACT_NAME`, `CONTRACT_VERSION`, `STATUS`, `CORRELATION_ID`, `SOURCE_REFERENCE`, `FAILURE_BRANCH`, `UPSTREAM_TYPE`, `DUPLICATE_SEGMENT_INDEX`, or `POLICY_MATCH_VIEW_VERSION` (**new in this successor, MV-1**) |
...
`ADMISSIBILITY` is valid in exactly two forms under this successor. The duplicate-segment-index
form (unchanged in every field except the contract identity/version, which now reads V2) is
exactly:
...
The Vision match-view admissibility form (new in this successor, MV-1/MV-2/MV-3/MV-4) is exactly:
...
It is emitted only after strict validation has produced a `VisionUnderstandingSuccessV1` whose
`policy_match_view_version` does not exactly equal `VISION_POLICY_MATCH_VIEW_VERSION`, and only
after the ASR admissibility invariant has already passed (ASR is checked before Vision). It is
emitted before correlation equality or typed upstream-status handling. **The observed
`policy_match_view_version` value is never exposed** — only the closed `field_code` token
identifies which field mismatched; no rejection at any phase carries the actual string a Vision
producer supplied. `VisionUnderstandingFailureV1` is never subject to this check (MV-2 = S2): a
Vision failure carrying any non-empty `policy_match_view_version`, canonical or not, proceeds
unaffected by this admissibility rule and is handled only by the existing typed-upstream-status
path.
...
prompts, credentials, endpoints, arbitrary metadata, unbounded strings, and (new in this
successor) the observed, non-canonical `policy_match_view_version` value.
...
is used for strict-validation failures and both T4 admissibility failures above;
`DUPLICATE_SEGMENT_INDEX` and `POLICY_MATCH_VIEW_VERSION` identify which of the two exact
admissibility forms applies, through `phase=ADMISSIBILITY` and `input_slot=ASR` or
`input_slot=VISION` respectively. `CORRELATION_MISMATCH` is the only correlation-phase code. No
code is emitted for a later phase after an earlier phase has rejected an input.
...
requiredness, nullability, and cross-field behavior. This successor changes no field in this
section; it only adds a T4-side admissibility comparison against the already-existing
`policy_match_view_version` field (section 3, section 4).
...
| `policy_match_view_version` | non-empty string | **for `VisionUnderstandingSuccessV1` only, MUST equal `VISION_POLICY_MATCH_VIEW_VERSION` (`vision-policy-match-view-v2`) exactly, enforced as the T4 admissibility invariant in section 3.3/section 4 (new in this successor, MV-1/MV-2/MV-3); a `VisionUnderstandingFailureV1` carrying any non-empty value is not checked (MV-2)** |
...
state. As stated above (MV-2 = S2), `policy_match_view_version` on a failure branch is not
subject to the section 3.3 admissibility comparison.
...
and `backend/src/sketch2life/contracts/schemas/vision.py:L120-L126,L365-L403`. The upstream
match-view constant `VISION_POLICY_MATCH_VIEW_VERSION` is declared at
`backend/src/sketch2life/contracts/schemas/vision.py:L16`.
...
mutable arbitrary map or metadata field. **Section 6 is entirely unchanged from revision 11**
except where this note calls out the section 6.1/2 identity update; the fused-result schema and
all fusion/matching/certainty semantics are untouched by this successor (MV note: "Fused-result
V1 and unrelated fusion semantics remain unchanged").
...
`P2T4FusionPolicyConfigV1` contains exactly these required, non-null fields (**unchanged from
revision 11**; `match_view_version` names the recipe and is not the admissibility comparator —
see sections 3.3 and 5.2 above for the comparator):
...
| `match_view_version` | literal string | `vision_policy_match_view-v2` (**unchanged; names the recipe, not the admissibility comparator**) |
...
`NOT_EXECUTED` or `BLOCKED`. It has no free-form error detail. (Its
`policy_match_view_version`, if any is carried at the source-envelope level, is unaffected by
this successor — see MV-2/section 5.2.)
...
- `FUSED` means both validated upstream models have `status=SUCCEEDED`, **which now additionally
  requires that every T4 admissibility invariant (ASR duplicate-index and, for a Vision success,
  the match-view invariant) already passed** — including an empty but schema-valid transcript and
  empty Vision collections. It has `upstream_failure=null`.
- `UPSTREAM_FAILURE` means one or both validated upstream models have `status=FAILED`, **and
  every admissibility invariant applicable to the successful side, if any, already passed** (a
  `VisionUnderstandingFailureV1` is never itself subject to the match-view invariant, per MV-2).
  It has the structurally matching non-null failure reference and empty entities, actions,
  relations, themes, conflicts, and uncertainty rows.
- Identity mismatch, strict-validation error, admissibility failure (either the ASR or the Vision
  match-view form), or correlation mismatch returns the `P2T4FusionInputRejectionV2` type and
  never produces a fused-result status.
...
- A successful ASR empty transcript is not an error, creates no narration-only
  candidate, and remains eligible for `FUSED` when Vision also succeeds (and passes the Vision
  admissibility invariant when Vision itself is a success).

## 7. Matching, negation, coordinates, and multi-span semantics

**Section 7 is entirely unchanged from revision 11.** The match-view *recipe* it describes
(`vision_policy_match_view-v2`, section 7.2) is the T4-internal token-normalization function used
for narration matching; it is a different concept from the *admissibility comparator* introduced
in section 3.3/section 4, which compares the upstream-declared `policy_match_view_version`
field's value to the upstream constant. Both concepts share the letters "match view" but serve
different purposes: the recipe transforms text into tokens; the admissibility invariant checks
that the Vision producer declared the same match-view generation as the one T4 assumes when it
applies that recipe.
...
**Section 8 is entirely unchanged from revision 11.** Fused-result V1 and every unrelated fusion
semantic — confidence, primary selection, ranking, grouping, and evidence cardinality — remain
unchanged by this successor.
...
**Section 9 is unchanged from revision 11.** The canonical-JSON projection, float/datetime/enum
rules, array sort keys, and conflict-ID byte algorithm apply identically to
`P2T4FusionInputRejectionV2` instances as they applied to `P2T4FusionInputRejectionV1` instances —
this section already describes a general model-projection algorithm keyed off declared fields,
not off a specific class name, so no rule text changes; only the concrete field values (the
literal `contract_name`/`contract_version` strings, and the new `field_code` token) change as a
consequence of sections 2 and 4 above.
...
digest suffix is always 64 lowercase hexadecimal characters. (Conflict IDs are unaffected by
this successor: they are computed only for fused observations inside `P2T4FusedResultV1`, never
for a `P2T4FusionInputRejectionV2`.)

## 10. Evidence binding, fixtures, and independent parity

### 10.1 Evidence binding record

Every future remediation-implementation evidence bundle must contain a hand-authored
...
| `predecessor_freeze_sha256` | normalized SHA-256 of revision 11 at its own path, confirmed byte-identical and untouched by this successor | `be96b32aa675b7b6e46eea30effb2dbb91c718dc68ba7ce36d4d627ad6058ee2` |
| `predecessor_package_sha256` | normalized SHA-256 of package revision 15 at its own path, confirmed byte-identical and untouched by this successor | `6821755722daf3bce622fe98eaf39124adb835c6854661143d79f48a943030d7` |
| `freeze_draft_sha256` | lowercase SHA-256 of this document's normalized UTF-8 bytes after removing this binding table and the revision-history section (this successor's own digest) | `d592b135d2d8a90024d48d1b8335321660e8d7f089873e48687707c760e3d3e9` |
| `implementation_package_sha256` | lowercase SHA-256 of the companion package revision 16's normalized UTF-8 bytes after removing its binding table and revision-history section | `ad886d261d2807bcc95264d05d6a385dc79cd2f0cad44e2b4f004531b8097d4f` |
| `dependency_lock_sha256` | lowercase SHA-256 for every declared dependency/lock input; unchanged from revision 11/15 because no dependency was added, removed, or repinned | `pnpm-lock.yaml=b406b4c36c1e5304cf0c43b175c426d50aa3b43dc9a2bb81be357ea2b80b1665`; `backend/pyproject.toml=9ca3a54905d11fdb7f30a84356d23741f256b2115b254bcc9fba4efacdf17df6` |
| `manifest_sha256` | SHA-256 of `features/FEAT-003-multimodal-understanding/fixtures/p2-t4-fusion-v1/manifest-v1.json` bytes, once the separately approved remediation updates it to this successor's `rejection_contract` and coverage | required once the remediated artifact exists; the current pre-remediation bytes at checkpoint `064ba62` are a superseded state and are not bound here |
...
| `expected_sha256` | SHA-256 of `features/FEAT-003-multimodal-understanding/fixtures/p2-t4-fusion-v1/expected-v1.json` bytes, once remediated | required once the remediated artifact exists; not bound to the pre-remediation bytes, whose 16 `REJECTED` entries still carry the superseded `P2T4FusionInputRejectionV1`/`1.0` identity |
...
own changing digest line. This docs-only successor does not claim fixture, expected,
or final-evidence hashes because the seven G2-approved paths have not yet been remediated to
this successor's semantics; the checkpoint `064ba62` bytes reflect the *prior*, defect-carrying
state and are deliberately not bound as if they already matched this document. Their required
fields and exact byte algorithm are nevertheless part of the proposed contract and
the future remediation gate, not an unresolved contract semantic.
...
### 10.2 Fixture and schema-parity requirements for the narrow remediation

The remediation-implementation allowlist is exactly these seven paths — the same seven paths
approved under G2 and already holding the G2-approved offline-core implementation at checkpoint
`064ba62`. No new path is added by this successor:
...
hand-authored from this successor and independently reviewed; they are not generated by
...
enum, literal, cross-field invariant, and rejection precedence rule for
`P2T4FusionInputRejectionV2` (superseding its `P2T4FusionInputRejectionV1` oracle entries). It
must not load a schema snapshot, serialize the implementation model, introspect implementation
fields, or generate expected schemas from `p2_t4_fusion.py`.

Because MV-4 replaces the outer rejection identity, the remediation's fixture impact is larger
than a pure addition: every one of the existing 16 `REJECTED` entries in `expected-v1.json`
currently carries `result.contract_name="P2T4FusionInputRejectionV1"` and
`result.contract_version="1.0"`; under this successor those become
`"P2T4FusionInputRejectionV2"`/`"2.0"`, which changes each of those 16 entries' canonical
projection and therefore each entry's `canonical_sha256`, even for the 15 whose `phase`/`code`/
`field_code` reason is otherwise unrelated to the match-view fix. The 26 non-rejected (`FUSED` or
`UPSTREAM_FAILURE`) entries are unaffected and must remain byte-identical, because neither their
outcome nor their content references the outer rejection type. `manifest-v1.json`'s
`rejection_contract` field must be updated from `P2T4.P2T4FusionInputRejectionV1@1.0` to
...
duplicate `AsrSegmentV1.index` rejection at the admissibility stage; **a non-canonical
`policy_match_view_version` on a `VisionUnderstandingSuccessV1` rejected at the admissibility
stage with `field_code=POLICY_MATCH_VIEW_VERSION`; the T4 policy literal
(`vision_policy_match_view-v2`) itself is not the upstream token and is also rejected when
supplied as the observed value, proving T1's exact-upstream-constant comparator rather than a
comparison against the policy literal; ASR duplicate-index admissibility checked before Vision
match-view admissibility when both invariants would otherwise fire; Vision match-view
admissibility checked before correlation equality; a `VisionUnderstandingFailureV1` carrying a
non-canonical `policy_match_view_version` is unaffected and still reaches `UPSTREAM_FAILURE`
(proving MV-2 = S2);** strict-validation
...
privacy sentinels (**including that the observed non-canonical `policy_match_view_version`
value never appears in any rejection, fixture record, or evidence artifact**); and
source/reference integrity. No mapping/adoption case is allowed.
...
payloads, prompts, credentials, endpoints, arbitrary metadata, real child data, or (new in this
successor) the observed, non-canonical `policy_match_view_version` string.

No fixture file is created or edited by this docs-only successor. Existing B0 compatibility
fixtures are not copied, extended, or relabeled as T4 implementation fixtures.

## 11. Governance and deferred boundaries

- The seven paths listed in section 10.2 are the exact, unchanged remediation-implementation
  allowlist; they already hold the G2-approved offline-core implementation (checkpoint `064ba62`)
  and are not a license to make unrelated changes.
- No file in that seven-path list may be created, edited, stubbed, or pre-populated
  while this successor is under review or awaiting owner freeze approval.
- No schema, code, test, fixture, runtime, provider, GPU, model, download, network,
  commit, push, or pull-request work is authorized by this successor document.
...
- FEAT-017, FEAT-018, P2-T2, P2-T3, their existing evidence and fixtures, and all
  previously approved immutable artifacts remain outside this task, including revision 11 and
  package revision 15 themselves, which are not edited in place.
- A separate successor-freeze decision (a new "G1 successor" approval) and a separate
  remediation-implementation approval remain required. This document cannot make the proposed
  `P2T4FusionInputRejectionV2` identity canonical by itself.

## 12. Independent review checklist and final handoff

### Required independent review (not yet performed)

This successor has not yet been independently reviewed. The following two passes are the
required review, to be performed as a separate step before any owner freeze decision on this
successor:

#### Pass 1 - technical completeness (pending)

- [ ] Active output identity (`P2T4.P2T4FusedResultV1@1.0`, unchanged), the new outer rejection
      identity (`P2T4.P2T4FusionInputRejectionV2@2.0`), historical Raw wording, and exact
      FEAT-018 rejection identities are synchronized.
- [ ] The Vision match-view admissibility invariant is stated exactly once, in the correct
      order (after the ASR invariant, before correlation and typed status), with the exact
      upstream comparator (`VISION_POLICY_MATCH_VIEW_VERSION`), and applies only to
      `VisionUnderstandingSuccessV1`.
- [ ] The two exact `ADMISSIBILITY` forms (duplicate-index and match-view) are both present, use
      `P2T4FusionInputRejectionV2`/`2.0`, and no mixed V1/V2 rejection union is describable.
- [ ] The observed `policy_match_view_version` value is never exposed anywhere in this document's
      rejection shapes, fixture-coverage list, or privacy-sentinel list.
- [ ] Per-segment source text, coordinate reset, short prefixes, duplicate spans, and no
      cross-segment matching are explicit and unchanged from revision 11.
- [ ] Exact match-view *recipe* (section 7.2) and the *admissibility comparator* (section 3.3)
      are clearly distinguished as different mechanisms.
- [ ] Mixed positive/refuting spans retain support and both canonical references while
      suppressing adjustment and primary eligibility (unchanged from revision 11).
- [ ] Complete field tables, requiredness/nullability, invariants, ranking, null ordering, and
      final source-ID tie-break are explicit and unchanged from revision 11 except section 4's
      identity/version and field-code table.
- [ ] Canonical projection, enum/tuple/null/datetime/float rules, array sort keys, Python 3.13
      algorithm, conflict bytes, delimiter, escaping, and lowercase SHA-256 are explicit and
      unchanged from revision 11.
- [ ] Evidence binding correctly distinguishes predecessor digests (revision 11/15, byte-frozen)
      from this successor's own digests, and does not bind fixture hashes to the pre-remediation
      (defect-carrying) checkpoint `064ba62` bytes.
- [ ] The fixture-coverage list includes the six new match-view scenarios listed in section 10.2,
      and states precisely which of the 42 existing fixture entries change bytes (the 16
      `REJECTED` entries) versus stay byte-identical (the 26 `FUSED`/`UPSTREAM_FAILURE` entries).

#### Pass 2 - governance and security (pending)

- [ ] Status is documentation-only and exactly **HOLD - NOT APPROVED**; this successor is not
      itself frozen or implementation-approved by being written.
- [ ] Revision 11 and package revision 15 remain byte-identical and untouched at their own
      paths; this successor is a new, separate file and does not overwrite them.
- [ ] The seven-file remediation-implementation list is exact, matches the existing G2 allowlist,
      and no implementation path was created, edited, stubbed, or pre-populated by this task.
- [ ] `P2T4_FEAT018_CONTRACT_FAMILY_MAPPING_V1@1.0` remains `PROPOSED_NOT_ADOPTED`.
- [ ] FEAT-018 adoption, mapping, preservation envelope, session/request/idempotency, registry,
      edge 3, Gate A, migration, runtime/provider/GPU/network work remain deferred.
- [ ] No credentials, raw transcript, media, provider payloads, absolute paths, or real child
      data are introduced.
- [ ] The five MV-1 through MV-5 decisions recorded in section 1.0 match
      `approvals/TASK_APPROVAL.md` exactly.
- [ ] Successor-freeze approval and remediation-implementation approval remain separate,
      not-yet-granted gates.

### Final task status

**HOLD - NOT APPROVED.** Independent review of this successor has not occurred. This document is
ready to be reviewed; being written does not constitute review, freeze, or approval.
...
| 11 | 2026-09-14 | Clarified mutually exclusive certainty precedence for conflicting and null-confidence candidates and refreshed normalized digest bindings; no freeze or implementation approval. **Approved as G1 at commit `18d0c33d35431ca96a76692a68c6b992098699e7` on 2026-09-15; remains immutable history at its own path (`plan/P2_T4_CONTRACT_FREEZE_DRAFT.md`) and is not edited by this or any later revision.** |
| 12 | 2026-09-15 | **Standalone successor** (this document, at a new path). Records the owner's five match-view decisions (MV-1=B, MV-2=S2, MV-3=T1, MV-4=V2, MV-5=standalone artifacts); adds the Vision match-view T4 admissibility invariant (section 3.3) checked after the ASR invariant and before correlation/typed status, for `VisionUnderstandingSuccessV1` only; replaces the outer rejection identity with `P2T4.P2T4FusionInputRejectionV2@2.0` for every outer-boundary rejection (section 2, section 4); adds the `POLICY_MATCH_VIEW_VERSION` field code; updates the evidence-binding table to the `064ba62` review base and records predecessor digests; documents that the remediation will change the canonical bytes of the 16 existing `REJECTED` fixture entries (identity/version fields only) while the 26 non-rejected entries stay byte-identical; leaves revision 11 byte-identical and untouched at its own path. Status remains `HOLD - NOT APPROVED`; no freeze or implementation approval is granted by this revision, and no independent review of it has yet occurred. |
